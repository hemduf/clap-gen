...
// Exact draft ABI IDs present in the CLAP SDK pinned by cmake/dependencies/Clap.cmake.
// Keep this list updated only together with the SDK pin.
...
fn merge_semantic_imports(
    ...
    for section in SECTIONS {
        ...
        for import in &loaded {
            append_section_children(&mut out, &import.metadata.document, section);
        }
        out.push_str("}\n");
    }
...
fn append_required_root_node(
    out: &mut String,
    document: &KdlDocument,
    name: &str,
) -> Result<(), String> {
    let node = document.get(name).ok_or_else(|| format!("missing `{name}` root node"))?;
    append_top_node(out, node);
    Ok(())
}

fn append_optional_root_node(out: &mut String, document: &KdlDocument, name: &str) {
    if let Some(node) = document.get(name) {
        append_top_node(out, node);
    }
}

fn append_top_node(out: &mut String, node: &KdlNode) {
...
fn append_section_children(out: &mut String, document: &KdlDocument, section: &str) {
    for root in document.nodes().iter().filter(|node| node.name().value() == section) {
        ...
        for child in children.nodes() {
            append_indented_text(out, &child.to_string());
        }
    }
}

fn append_indented_text(out: &mut String, text: &str) {
...
fn validate_parameter_semantics(
    path: &Path,
    source: &str,
    document: &KdlDocument,
) -> Result<(), String> {
        ...
        section_children(document, "parameters").filter(|node| node.name().value() == "param")
    {
        ...
        if bypass && !stepped {
            return Err(diagnostic(
                ...
                source,
                "param",
                &format!("bypass parameter `{id}` must also declare the `stepped` flag"),
                "use flags=\"bypass,stepped\"",
            ));
        }
        if enumeration && !stepped {
            return Err(diagnostic(
                ...
                source,
                "param",
                &format!("enum parameter `{id}` must also declare the `stepped` flag"),
                "use flags=\"enum,stepped\"",
            ));
        }
        if bypass {
            bypass_ids.push(id.to_owned());
            if number_prop(node, "min") != Some(0.0) || number_prop(node, "max") != Some(1.0) {
                return Err(diagnostic(
                    path,
                    source,
                    "param",
                    &format!("bypass parameter `{id}` must use min=0 and max=1"),
                    "CLAP bypass parameters have the fixed [0, 1] range",
                ));
            }
        }
    }
    ...
    if bypass_ids.len() > 1 {
        return Err(diagnostic(
            ...
            source,
            "param",
            &format!("only one bypass parameter is allowed; found {}", bypass_ids.join(", ")),
            "keep a single parameter with the `bypass` flag",
        ));
    }
    Ok(())
}

fn validate_main_audio_ports(
    path: &Path,
    source: &str,
    document: &KdlDocument,
) -> Result<(), String> {
    for direction in [PortDirection::Input, PortDirection::Output] {
        let mains = section_children(document, "audio-ports")
            ...
            .filter_map(|node| string_prop(node, "id"))
            .collect::<Vec<_>>();
        if mains.len() > 1 {
            return Err(diagnostic(
                path,
                ...
                direction.as_str(),
                &format!(
                    "only one main {} audio port is allowed; found {}",
                    direction.as_str(),
                    mains.join(", ")
                ),
                "CLAP requires at most one main port per direction",
            ));
        }
    }
    Ok(())
...
fn validate_pinned_extension_contract(
    path: &Path,
    source: &str,
    document: &KdlDocument,
) -> Result<(), String> {
        ...
        section_children(document, "extensions").filter(|node| node.name().value() == "enable")
    {
        ...
        if draft && !known_draft {
            return Err(diagnostic(
                path,
                source,
                "enable",
                &format!(
                    "draft extension `{id}` is not present in pinned CLAP SDK `{CLAP_SDK_PIN}`"
                ),
                "use an exact draft ABI ID from the pinned SDK or update the SDK pin explicitly",
            ));
        }
        if !draft && known_draft {
            return Err(diagnostic(
        ...
        if draft && !has_exact_version_pin(id, string_prop(node, "version")) {
            return Err(diagnostic(
                path,
                source,
                "enable",
                &format!("draft extension `{id}` must use its exact ABI version pin"),
                "make `version` match the numeric suffix in the extension ID",
            ));
        }
    }
    Ok(())
...
fn has_exact_version_pin(id: &str, version: Option<&str>) -> bool {
...
fn build_rewrite_plan(
    ...
    for node in section_children(document, "audio-ports") {
        let Some(direction) = PortDirection::from_node(node.name().value()) else {
            continue;
        };
        let Some(id) = string_prop(node, "id") else {
            continue;
        };
        let main = symbolic_list(node, "flags").iter().any(|flag| flag == "main");
        let internal =
            format!("__clapgen_audio_{}_{}_{}", i32::from(!main), direction.as_str(), hex(id));
        plan.audio_ids.insert((direction, id.to_owned()), internal.clone());
        plan.replacements.push((internal, id.to_owned()));
    }
    ...
    for node in section_children(document, "note-ports") {
        ...
        plan.note_ids.insert((direction, id.to_owned()), internal.clone());
        plan.replacements.push((internal, id.to_owned()));
    }

    for node in
        section_children(document, "note-ports").filter(|node| node.name().value() == "note-name")
    {
        let Some(target) = string_prop(node, "port") else {
            continue;
        };
        let matches = plan.note_ids.keys().filter(|(_, id)| id == target).count();
        if matches > 1 {
            ...
            return Err(diagnostic(
                ...
                "note-name",
                &format!(
                    "note-name `{name}` has ambiguous note port target `{target}` because that ID exists in both directions"
                ),
                "use distinct note port IDs when note-name metadata references a port",
            ));
        }
    }
        ...
        section_children(document, "extensions").filter(|node| node.name().value() == "enable")
    {
        ...
        let draft = bool_prop(node, "draft").unwrap_or(false);
        if !draft && id.starts_with("clap.") && id.contains('/') {
            let internal = format!("__clapgen_stable_extension_{}", hex(id));
            plan.stable_extension_ids.insert(id.to_owned(), internal.clone());
            plan.replacements.push((internal, id.to_owned()));
        }
    }
...
fn render_audio_ports(out: &mut String, root: &KdlNode, plan: &RewritePlan) {
    ...
    for node in children.nodes() {
        ...
        let id = string_prop(node, "id").unwrap_or("");
        let internal = plan.audio_ids.get(&(direction, id.to_owned())).map_or(id, String::as_str);
        out.push_str("    ");
        out.push_str(direction.as_str());
        if let Some(name) = first_string(node).or_else(|| string_prop(node, "name")) {
            out.push_str(" name=");
            out.push_str(&quote(name));
        }
        out.push_str(" id=");
        out.push_str(&quote(internal));
        if let Some(channels) = prop(node, "channels") {
            out.push_str(" channels=");
            out.push_str(&value_text(channels));
        }
        append_optional_string_prop(out, node, "type");
        append_optional_string_prop(out, node, "flags");
        if let Some(target) = string_prop(node, "in-place-pair") {
            let rewritten = plan
                .audio_ids
                .get(&(direction.opposite(), target.to_owned()))
                .map_or(target, String::as_str);
            out.push_str(" in-place-pair=");
...
fn render_note_ports(out: &mut String, root: &KdlNode, plan: &RewritePlan) {
    ...
    for node in children.nodes() {
        if let Some(direction) = PortDirection::from_node(node.name().value()) {
            let id = string_prop(node, "id").unwrap_or("");
            let internal =
                plan.note_ids.get(&(direction, id.to_owned())).map_or(id, String::as_str);
            out.push_str("    ");
            out.push_str(direction.as_str());
            if let Some(name) = first_string(node).or_else(|| string_prop(node, "name")) {
                out.push_str(" name=");
            ...
            out.push_str(" id=");
            out.push_str(&quote(internal));
            append_optional_string_prop(out, node, "dialects");
            append_optional_string_prop(out, node, "preferred");
            out.push('\n');
            continue;
        }
        ...
        if node.name().value() == "note-name" {
            ...
            append_optional_value_prop(out, node, "key");
            append_optional_value_prop(out, node, "channel");
            if let Some(target) = string_prop(node, "port") {
                let mut matches = plan
                    .note_ids
                    .iter()
                    .filter(|((_, id), _)| id == target)
                    .map(|(_, internal)| internal.as_str());
                let rewritten = matches.next().unwrap_or(target);
                out.push_str(" port=");
                out.push_str(&quote(rewritten));
            }
            out.push('\n');
...
fn render_extensions(out: &mut String, root: &KdlNode, plan: &RewritePlan) {
    ...
    for node in children.nodes() {
        ...
        let rewritten = plan.stable_extension_ids.get(id).map_or(id, String::as_str);
        out.push_str("    enable ");
        out.push_str(&quote(rewritten));
        append_optional_string_prop(out, node, "version");
        if let Some(draft) = bool_prop(node, "draft") {
            out.push_str(" draft=");
            out.push_str(if draft { "#true" } else { "#false" });
        }
        out.push('\n');
    }
    out.push_str("}\n");
...
fn append_optional_string_prop(out: &mut String, node: &KdlNode, name: &str) {
    if let Some(value) = string_prop(node, name) {
        out.push(' ');
        out.push_str(name);
        out.push('=');
...
fn append_optional_value_prop(out: &mut String, node: &KdlNode, name: &str) {
    if let Some(value) = prop(node, name) {
        out.push(' ');
        out.push_str(name);
        out.push('=');
...
fn number_prop(node: &KdlNode, key: &str) -> Option<f64> {
    match prop(node, key)? {
...
fn symbolic_list(node: &KdlNode, key: &str) -> Vec<String> {
...
fn restore_text(mut text: String, replacements: &[(String, String)]) -> String {
    for (internal, original) in replacements {
